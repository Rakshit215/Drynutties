    ...
    public void postFetch(final String url, final Map<String, String> params, final Task task, final int method) {
        ...
        StringRequest myRequest = new StringRequest(method, getAbsoluteUrl(url), new Response.Listener<String>() {
            ...
            public Map<String, String> getHeaders() throws AuthFailureError {
                ...
                SdkLogger.i("SDK Cookie -- "+SdkHelper.getUserCookieSessionId(mContext));
...
*/
    ...
    private boolean checkForValidObject(JSONObject jsonObject, String paymentModeString) {
        try {
            if (jsonObject.has(paymentModeString)
                    ...
                    && !("false").equals(jsonObject.getString(paymentModeString))
    ...
    public void sendToPayU(JSONObject details, final String mode, final HashMap<String, Object> data, Double cashback1, Double discount) throws JSONException {
        ...
        if (cashback != null && cashback.floatValue() == 0) {
            ...
            if (checkForValidObject(details, "convenienceCharges")) {
                JSONObject convenienceChargesJsonObject = new JSONObject(details.getString("convenienceCharges"));
                ...
                if (checkForValidObject(convenienceChargesJsonObject, currentMode)) {
                    convenienceChargesAmount = convenienceChargesJsonObject.getJSONObject(currentMode).getDouble("DEFAULT");
                }
            }

            Double payUAmt = Double.valueOf(details.getJSONObject(SdkConstants.PAYMENT).getDouble("orderAmount") + convenienceChargesAmount - Dis_amt.doubleValue());
